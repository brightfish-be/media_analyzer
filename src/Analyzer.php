...
class Analyzer
{
    ...
    public function analyze(string $path)
    {
         ...
         */
        $result=Array();
        $result["ffmpeg"]["version"]=str_replace(Array(" the FFmpeg developers","Copyright "),"",$lines[0]);
        ...
        foreach($lines as $line){
            ...
            if(strstr($line,"Stream #")){
                ...
                if(strstr($line,"Video:")){
                    ...
                    if($result["video"]["size"]){
                        ...
                        switch($result["video"]["aspect"]){
                            case 1.78:  $result["video"]["aspect_type"]="HD"; break;
                            case 1.90:  $result["video"]["aspect_type"]="DCP"; break;
                            case 1.85:  $result["video"]["aspect_type"]="FLAT"; break;
                            case 2.35:  $result["video"]["aspect_type"]="SCOPE"; break;
                            case 2.39:  $result["video"]["aspect_type"]="SCOPE"; break;
                            case 1.33:  $result["video"]["aspect_type"]="TV"; break;
                        }
                    }
                    $result["video"]["fps"]=(double)$this->find($data,"|([\d\.]+) fps|");
                    if(!$result["video"]["fps"]) $result["video"]["fps"]=$this->find($data,"|([\d\.]+) tbr|");
                ...
                if(strstr($line,"Audio:")){
                    ...
                    if(!$achannels AND strstr($data,"stereo")) $achannels=2;
                    if(!$achannels AND strstr($data,"5.1")) $achannels=6;
                    if(!$achannels)  $achannels=1;
                    ...
                    if(!$result["audio"]["bits"])  $result["audio"]["bits"]=16;
                    $result["audio"]["hertz"]=$this->find($data,"|(\d\d\d\d\d) hz|");
                    if(!$result["audio"]["hertz"])  $result["audio"]["hertz"]=48000;
                    ...
                    switch(true){
                        case($result["audio"]["hertz"]<44000): $result["audio"]["quality"]="low"; break;
                        case($result["audio"]["bits"]<16): $result["audio"]["quality"]="low"; break;
                        case($result["audio"]["compression"] < 0.1): $result["audio"]["quality"]="low"; break;
